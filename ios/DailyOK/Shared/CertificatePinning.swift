...
enum CertificatePinning {
    ...
    private static func asn1Header(for key: SecKey) -> [UInt8]? {
        ...
        // Hoist the CFString key-type constants into String locals. Writing the
        // cast inline in a case pattern (`case (kSecAttrKeyTypeRSA as String, …)`)
        // makes the Swift 6 compiler parse `as String` as a cast *pattern* rather
        // than a bridging expression, and reject it with "expression pattern of
        // type 'CFString' cannot match values of type 'String'". A local `let`
        // makes each case a plain value pattern matched with `==`.
        let rsaKey = kSecAttrKeyTypeRSA as String
        let ecKey = kSecAttrKeyTypeECSECPrimeRandom as String
