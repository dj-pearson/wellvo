...
struct CheckInReportGenerator {
    ...
    static func generatePDF(from data: ReportData) -> Data {
        ...
        let pdfData = pdfRenderer.pdfData { context in
            ...
            //
            // The summary block above has always been bucketed in the
            // receiver's zone; this table was not, so it rendered in the
            // owner's. An owner in London printing a report for a parent in
            // Chicago got a 9pm Chicago check-in listed as 3am the NEXT day —
            // the daily log disagreeing with the streak and day counts printed
            // six inches above it, in a document that goes to a doctor.
            let dateFmt = DateFormatter()
            dateFmt.calendar = reportCalendar
            dateFmt.timeZone = reportCalendar.timeZone
            dateFmt.dateStyle = .medium
            dateFmt.timeStyle = .none
            let timeFmt = DateFormatter()
            timeFmt.calendar = reportCalendar
            timeFmt.timeZone = reportCalendar.timeZone
