...
struct FloatingBottomNav: View {
    ...
    var body: some View {
        ...
        .wellvoShadow(WellvoElevation.level4, opacity: 0.12)
...
private struct NavItemView: View {
    ...
    var body: some View {
            ...
            ZStack(alignment: .topTrailing) {
                ...
                if item.badgeCount > 0 {
                    Text("\(item.badgeCount)")
                        ...
                        .padding(.vertical, 1)
                        .background(Capsule().fill(WellvoColor.error))
                        .offset(x: 8, y: -6)
                }
            }
            if selected {
        ...
        .foregroundStyle(selected ? WellvoColor.green700 : Color.secondary)
        .padding(.horizontal, selected ? 14 : 10)
        .padding(.vertical, 10)
        .background(
            RoundedRectangle(cornerRadius: 22, style: .continuous)
                .fill(selected ? WellvoColor.green100 : Color.clear)
        )
