        ...
        VStack(spacing: 16) {
            ...
            HStack(spacing: 8) {
                ForEach(0..<features.count, id: \.self) { i in
                    Capsule()
                        .fill(i == currentIndex ? WellvoColor.brand : Color(.systemGray4))
                        .frame(width: i == currentIndex ? 24 : 8, height: 8)
                        .animation(WellvoMotion.smoothSpring, value: currentIndex)
                }
            }
        }
    }
    ...
    var body: some View {
        VStack(spacing: 16) {
            ZStack {
                ...
                Image(systemName: feature.systemImage)
                    ...
                    .foregroundStyle(WellvoColor.green700)
            }
        ...
        .background(
            RoundedRectangle(cornerRadius: 20, style: .continuous)
                .fill(LinearGradient(
                    colors: [WellvoColor.green50, WellvoColor.green100],
                    startPoint: .top,
                    endPoint: .bottom
                ))
        )
...
struct AnnualSavingsBadge: View {
    ...
    var body: some View {
        Text("Save \(percentSaved)%")
            ...
            .foregroundStyle(WellvoColor.gold)
            .padding(.horizontal, 10)
            .padding(.vertical, 5)
            .background(
                RoundedRectangle(cornerRadius: 8, style: .continuous)
                    .fill(WellvoColor.goldLight)
            )
    }
}
