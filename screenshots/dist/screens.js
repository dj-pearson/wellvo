...
function getStatusBarHTML(device) {
...
function getScreenStyles(screenId, device) {
    const isIPad = device.slug.startsWith('ipad');
...
function getScreenContent(screenId, device) {
    const isIPad = device.slug.startsWith('ipad');
    switch (screenId) {
        case 'receiver-checkin':
            return `
...
function generateTabBar(active) {
    ...
    return `<div class="tab-bar">${tabs.map(t => `<div class="tab-item ${t.id === active ? 'active' : ''}">${t.icon}<span>${t.label}</span></div>`).join('')}</div>`;
}
...
function generateReceiverCards(isIPad) {
    const receivers = [
...
function generateHeatmapCells(isIPad) {
...
function generateChartSVG(isIPad) {
