...
class CheckInNotificationReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        val notificationId = intent.getIntExtra(DailyOKFirebaseMessagingService.EXTRA_NOTIFICATION_ID, -1)
        ...
        when (intent.action) {
            DailyOKFirebaseMessagingService.ACTION_CHECKIN_OK,
            DailyOKFirebaseMessagingService.ACTION_CHECKIN_NEED_HELP,
            DailyOKFirebaseMessagingService.ACTION_CHECKIN_CALL_ME -> {
                handleCheckInAction(context, intent)
            }
            DailyOKFirebaseMessagingService.ACTION_CALL_NOW -> {
    ...
    private fun handleCheckInAction(context: Context, intent: Intent) {
        val requestId = intent.getStringExtra(DailyOKFirebaseMessagingService.EXTRA_REQUEST_ID)
        if (requestId == null) {
    ...
    private fun handleCallNow(context: Context, intent: Intent) {
        val receiverId = intent.getStringExtra(DailyOKFirebaseMessagingService.EXTRA_RECEIVER_ID)
        if (receiverId == null) {
